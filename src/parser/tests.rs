...
fn test_int() {
    ...
    assert_eq!(parse("1234isize"), Ok(EuFn::from([EuIsize(1234).into()])));
}

#[test]
fn test_float() {
    assert_eq!(parse("1234.5"), Ok(EuFn::from([EuF64(1234.5).into()])));
    assert_eq!(parse(".1234"), Ok(EuFn::from([EuF64(0.1234).into()])));
    assert_eq!(parse("1234.0"), Ok(EuFn::from([EuF64(1234.0).into()])));
    assert_eq!(parse("123e4"), Ok(EuFn::from([EuF64(123e4).into()])));
    assert_eq!(parse("123e-4"), Ok(EuFn::from([EuF64(123e-4).into()])));
    assert_eq!(parse("123.e4"), Ok(EuFn::from([EuF64(123e4).into()])));
    assert_eq!(parse("12.34e5"), Ok(EuFn::from([EuF64(12.34e5).into()])));
    assert_eq!(parse("123f32"), Ok(EuFn::from([EuF32(123.0).into()])));
    assert_eq!(parse("123.f32"), Ok(EuFn::from([EuF32(123.0).into()])));
    assert_eq!(parse("123e-4f32"), Ok(EuFn::from([EuF32(123e-4).into()])));
}
...
#[test]
fn test_word() {
    assert_eq!(parse("asdf"), Ok(EuFn::from([EuWord::from("asdf").into()])));
    assert_eq!(
        parse("asdf1234"),
...
fn test_all() {
    ...
    assert_eq!(
        parse("123ever"),
        Ok(EuFn::from([EuI64(123).into(), EuWord::from("ever").into()]))
    );
    assert_eq!(
        parse("123e.4"),
        Ok(EuFn::from([EuI64(123).into(), EuWord::from("e.4").into()]))
    );
    assert_eq!(
        parse("(1 2+)map"),
        Ok(EuFn::from([
...
fn parse(input: &str) -> Result<EuFn<'_>, ParseError<&str, ContextError>> {
